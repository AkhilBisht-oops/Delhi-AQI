...
import { Globe, Github, Twitter, Heart, Mail, MapPin, ExternalLink } from 'lucide-react';
    ...
    <footer className="border-t border-white/5 bg-[#050a18]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-12 grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Quick Links */}
          <div>
            <h3 className="text-sm font-bold text-white uppercase tracking-wide mb-4">Navigation</h3>
          ...
          {/* AQI Quick Reference */}
          <div>
            <h3 className="text-sm font-bold text-white uppercase tracking-wide mb-4">AQI Scale</h3>
            <div className="space-y-2">
              {[
                { range: '0-50', label: 'Good', color: '#00e400' },
                { range: '51-100', label: 'Moderate', color: '#ffde33' },
                { range: '101-150', label: 'Unhealthy (Sensitive)', color: '#ff9933' },
                ...
                <div key={item.range} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-gray-400">
                    <span className="font-semibold text-gray-300">{item.range}</span> {item.label}
                  </span>
                </div>
              ))}
            </div>
